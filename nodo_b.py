"""Nodo B independiente.

Proceso propio, reloj vectorial propio y API REST en el puerto 5002.
Se comunica con A y C solo por HTTP/JSON. No comparte memoria con los otros nodos.
"""
...
logger = logging.getLogger("nodo_b")
...
def event():
    data = request.get_json(silent=True) or {}
    return jsonify({"ok": True, "evento": nodo.local_event(data.get("descripcion", "evento local en B"))})
...
if __name__ == "__main__":
    logger.info("Nodo B (nodo_b.py) escuchando en puerto %s", PUERTO)
